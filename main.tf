...
resource "aws_ecr_repository" "first_ecr_repo" {
  name = "first_ecr_repo" # Naming my repository
}
resource "aws_ecs_cluster" "first_cluster" {
  name = "fisrt-cluster" # Naming the cluster
}
...
#correct permissions to execute
resource "aws_iam_role_policy_attachment" "ecsTaskExecutionRole_policy" {
  role       = "${aws_iam_role.ecsTaskExecutionRole.name}"
  policy_arn = "arn:aws:iam::aws:policy/service-role/AmazonECSTaskExecutionRolePolicy"
}
resource "aws_alb" "application_load_balancer" {
  name               = "hello-lb-tf" # Naming our load balancer
...
resource "aws_ecs_service" "first_service" {
  name            = "first-service"                             # Naming our first service
  cluster         = "${aws_ecs_cluster.first_cluster.id}"  
  #aws_ecs_cluster" "first_cluster           # Referencing our created Cluster
  task_definition = "${aws_ecs_task_definition.first_task.arn}" # Referencing the task our service will spin up
  launch_type     = "FARGATE"
